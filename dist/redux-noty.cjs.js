...
var NotyLib = _interopDefault(require('noty'));
...
var reset = reduxStarterKit.createAction('noty/RESET');
var add = reduxStarterKit.createAction('noty/ADD');
var remove = reduxStarterKit.createAction('noty/REMOVE');
var removeAll = reduxStarterKit.createAction('noty/REMOVE_ALL');
...
function payloadToOpts(type, p) {
  ...
  if (p.length === 1) {
    if (lodashEs.isPlainObject(p[0])) options = p[0];
    if (lodashEs.isString(p[0])) text = p[0];
  } else if (p.length === 2) {
    if (lodashEs.isPlainObject(p[1])) {
      text = p[0];
      options = Object.assign({}, p[1]);
    }

    if (lodashEs.isString(p[1])) {
      title = p[0];
      text = p[1];
    }
  } else if (p.length === 3) {
  ...
  if (lodashEs.includes(['alert', 'success', 'warning', 'error', 'info'], type)) options.type = type;
  return options;
}
...
function Noty(_ref2) {
  ...
  return React__default.createElement(React__default.Fragment, null, items.map(function (i, k) {
    return React__default.createElement(Item, _extends({
      key: k,
      remove: remove
    }, i));
  }));
...
var withConnect = reactRedux.connect(mapStateToProps, mapDispatchToProps);
var Noty$1 = redux.compose(withConnect, React.memo)(Noty);

var getNoty = reduxStarterKit.createSelector(['noty'], function (noty) {
  return noty;
});
...
var reducer = reduxStarterKit.createReducer(initialState, (_createReducer = {}, _defineProperty(_createReducer, add, function (state, _ref) {
...
exports.default = Noty$1;
exports.noty = actions$1;
exports.reducer = reducer;
exports.selector = selectors;
